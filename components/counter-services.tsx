...
const CounterServices = () => {
    ...
    return (
        <div className="relative mx-auto my-10 grid max-w-[1180px] grid-cols-2 gap-3 sm:gap-5 md:my-12 md:flex md:gap-8 xl:my-14">
            ...
            <div className="absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-contrast/40 md:hidden" />

            {dataCounter.map(({ id, endCounter, text, lineRight }) => (
                <div
                    key={id}
                    className={lineRight ? "md:border-r md:border-contrast/50 md:pr-8" : ""}
                >
